...
class Task {
	...
	public function getTasksFromList($listId){
		$qForTaskFromList = "SELECT `tasks`.`id`, `tasks`.`title`, `tasks`.`description`, `tasks`.`expiredBy`, `tasks`.`completed` FROM `lists` LEFT JOIN `tasks` ON (`lists`.`id` = `tasks`.`listId`) WHERE `lists`.`id` = ? AND `tasks`.`id` != 'NULL' ORDER BY `active` DESC, `id` DESC LIMIT ".$_SESSION['start'].", ".$_SESSION['ipp']."";
